...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      setError('O sistema não está configurado corretamente (Supabase URL ausente).');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log('Resposta do Supabase recebida com sucesso. Sessão:', data.session ? 'Presente' : 'Ausente');

      if (data.session) {
        // Required for iframe compatibility in AI Studio preview
        ...
        console.log('Cookie de sessão definido. Redirecionando para a Home...');
        router.push('/');
        // Forçar um refresh caso o router.push demore em alguns ambientes
        setTimeout(() => {
          window.location.href = '/';
        }, 500);
      } else {
        setError('Sessão não iniciada. Verifique se sua conta está ativa e o e-mail confirmado no Supabase.');
        setIsLoading(false);
      }
    } catch (err: any) {
