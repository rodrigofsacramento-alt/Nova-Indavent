...
export function Sidebar() {
  ...
  const handleLogout = () => {
    // Usando a função logout do useAuth para garantir limpeza completa
    logout();
  };
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 w-64 border-r border-slate-800 bg-slate-950 flex flex-col h-screen transition-transform duration-300 ease-in-out lg:translate-x-0",
        isOpen ? "translate-x-0" : "-translate-x-full"
      )}>
        <div className="flex flex-col h-full">
          {/* Brand */}
          <div className="flex items-center justify-between lg:justify-center mb-8 px-4 pt-6">
            ...
            <button 
              onClick={close}
              className="lg:hidden p-2 text-slate-400 hover:bg-slate-900 rounded-lg transition-colors absolute right-4 top-4"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="pt-6 border-t border-slate-800 space-y-4">
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-lg transition-colors"
            >
              <Plus size={18} />
            ...
            <div className="flex items-center gap-3 p-2 mt-4 bg-slate-900/50 rounded-xl border border-slate-800">
              <div className="size-9 rounded-full bg-blue-600 flex items-center justify-center text-white font-black text-sm uppercase ring-2 ring-slate-800 relative overflow-hidden">
                {profile?.avatar_url ? (
                  <Image 
